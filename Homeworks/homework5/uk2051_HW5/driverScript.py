...
def RunSimulationQuestion_64(beta, L, M, h, initialValues, scaleupConst):
    ...
    out = Parallel(n_jobs = 1, verbose = 10)(delayed(GenerateSamplesFromOverdampled)(beta, L, M, h, method, initialValues, scaleupConst) for method in method_name)
    return out

def RunSimulationQuestion_65(beta, L, M, h, initialValues, scaleupConst, n = 100):
    method_name = ['notM', 'M']
    #First one is without notMH and second one is with MH
    out = Parallel(n_jobs = 1, verbose = 10)(delayed(GenerateSamplesFromHybrid)(beta, L, M, h, method, initialValues, scaleupConst, n) for method in method_name)
    return out

def RunSimulationQuestion_66(beta, L, M, h, gamma, initialValues, scaleupConst):
    method_name = ['notM', 'M']
    #First one is without notMH and second one is with MH
    out = Parallel(n_jobs = 1, verbose = 10)(delayed(GenerateSampleFromUnderdamped)(beta, L, M, h, method, initialValues, scaleupConst, gamma) for method in method_name)
    return out
...
def RunTestsOnQuestion_65(beta, L, M, initialValues, hs, scaleupConst, n = 100):
    out = Parallel(n_jobs = 3, verbose = 10)(delayed(RunSimulationQuestion_65)(beta, L, M, h, initialValues, scaleupConst, n) for h in hs)
    return out
...
def RunTestsOnQuestion_66(beta, L, M, initialValues, hs, gammas, scaleupConst):
    out = Parallel(n_jobs= 3, verbose= 10)(delayed(MidRunForGammas)(beta, L, M, initialValues, h, gammas, scaleupConst)for h in hs)
    return out
...
def PlotGraphsForAllModelsIndividuallyForExercise66(output, exerciseName):
    out = np.array(output)
    rows, cols, types = out.shape
    for g in range(types):
        for r in range(rows):
            for c in range(cols):
                curr_model = out[r,c, g]
                totalNs = len(curr_model.IAT_sliceIndexOnePast)
                nameToUse = exerciseName + '_G_' + str(curr_model.gamma)
                for i in range(totalNs):
                    curr_model.GenerateIATGraphs(nameToUse, i)
                    curr_model.GenerateHistograms(nameToUse, i)
...
hs = [1/2., 1/4., 1/8.]
gammas = [0.001, 0.01]
scaleupConst = 1e5 #No of samples for h == 1