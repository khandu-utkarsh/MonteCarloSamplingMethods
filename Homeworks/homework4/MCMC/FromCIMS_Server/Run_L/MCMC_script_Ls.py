...
def MethodsParallelized(initSpins, L, beta, N_max):
    methods = ['G', 'MH']
    trueOut = Parallel(n_jobs=2,verbose=10)(delayed(TypesParallelized)(initSpins, L, beta, N_max, method) for method in methods)
    return trueOut

def LParallelized(Ls, beta, N_max):
    lOut = Parallel(n_jobs=len(Ls),verbose=10)(delayed(MethodsParallelized)(np.random.randint(0, 2, size=(L, L)) * 2 - 1, L, beta, N_max) for L in Ls)
    return  lOut
