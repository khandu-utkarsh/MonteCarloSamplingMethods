...
def ComputeVarianceOfSamples(L,M,N, initialSpins,beta = 0.05, resampling=False):
    ...
    timeTaken = end - start
    return magnetization, average, variance, timeTaken

def GetVarianceOfSamples(L,M,N, initialSpins,beta = 0.05, resampling=False):
    magnetization, average, variance, timeTaken = ComputeVarianceOfSamples(L, M, N, initialSpins, beta, resampling)
    # start = time.perf_counter()
    # model = Jarzynski(beta,L,M,N)
    # model.InitializeLattice(initialSpins)
    # magnetization, weights = model.GenerateMagnetizationSamples(resampling)
...
def Run(L,M,beta,resampling):
    ...
    Ns = [100000,200000,300000,400000,500000,600000,700000,800000,900000,1000000]
